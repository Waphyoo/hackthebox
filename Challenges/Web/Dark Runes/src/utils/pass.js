...
const rotatePass = () => {
    try {
        ...
        console.log("New access pass generated:", ACCESS_PASS);
        fs.writeFileSync(
            String(ACCESS_PASS),
...
const verifyPass = (pass) => {
    console.log("Verifying pass:", pass);
    try {
        if (!fs.existsSync(ACCESS_PASS)) {
            console.log(ACCESS_PASS, "does not exist");
            return false;
        }
        ...
        if (currName.length === 0) {
            console.log("Access pass file is empty");
            return false;
        }
