...
# PHP Web Shells - Web Shell ที่ได้นิยมมากที่สุด
...
- **General-purpose** ใช้งานได้หลากหลาย
- **Server-side Programming**

**2. การใช้งาน:**
- เป็นส่วนหนึ่งของ **Web Stack**
- ใช้ขับเคลื่อนเว็บไซต์
- ประมวลผลฝั่ง Server

### สถิติความนิยม (ณ ตุลาคม 2021):

**ภาษา Server-side ที่ได้นิยมมากที่สุด!**

📊 **ตามสำรวจโดย W3Techs:**
> "PHP ถูกใช้โดย **78.6% ของเว็บไซต์ทั้งหมด** ที่เรารู้ว่าใช้ภาษา Server-side"

**หมายความว่า:**
- เกือบ 8 ใน 10 เว็บไซต์ใช้ PHP
- มีโอกาสพบ PHP Server สูงมากใน Pentest
- เป็น Attack Surface ที่สำคัญ

## ตัวอย่างการทำงานของ PHP

### กรณีศึกษา: rConfig Login Page

**Scenario:**
- เว็บไซต์มีหน้า Login (login.php)
- ผู้ใช้กรอก Username และ Password
- กดปุ่ม Login

**กระบวนการ:**
1. 📝 ผู้ใช้กรอกข้อมูลใน Form
2. 🔄 ข้อมูลถูกส่งไปยัง Server
3. ⚙️ **PHP ประมวลผลฝั่ง Server**
4. ✅ ตรวจสอบ Credentials
5. 📤 ส่งผลลัพธ์กลับมา

**สิ่งที่ Pentester ต้องรู้:**
> เมื่อรู้ว่า Web Server ใช้ PHP → มีโอกาสได้ **PHP-based Web Shell** บนระบบนี้!

## การใช้งาน PHP Web Shell แบบ Hands-on

### เป้าหมาย: rConfig 3.9.6

**ช่องโหว่:**
- rConfig 3.9.6 มีช่องโหว่ File Upload
- สามารถอัปโหลด PHP Web Shell ได้

**ฟังก์ชันของ rConfig:**
- Admin สามารถเพิ่ม Network Devices
- จัดหมวดหมู่ตาม Vendor
- อัปโหลด **Vendor Logo** ได้
...
**3. เตรียม Payload:**
- ใช้ **WhiteWinterWolf's PHP Web Shell**
- ดาวน์โหลดหรือ Copy Source Code
- บันทึกเป็นไฟล์ `.php`

**⚠️ File Type สำคัญ!**
- ต้องเป็นไฟล์ `.php`
- Server จะประมวลผล PHP Code

## การ Bypass File Type Restriction

### ปัญหา:

**rConfig มีการตรวจสอบ File Type:**
- ยอมรับเฉพาะไฟล์รูปภาพ
  - `.png`
  - `.jpg`
  - `.gif`
  - etc.
- ❌ **ปฏิเสธไฟล์ .php โดยตรง**

### วิธีแก้: ใช้ Burp Suite

**ขั้นตอนที่ 1: ตั้งค่า Burp Suite**

**1.1 เปิด Burp Suite**

**1.2 ตั้งค่า Browser Proxy:**
```
...
**1.3 วัตถุประสงค์:**
- ให้ Request ทั้งหมดผ่าน Burp
- Burp ทำหน้าที่เป็น **Web Proxy**
- สามารถ Intercept และแก้ไข Request ได้

**ขั้นตอนที่ 2: Upload และ Intercept**

**2.1 Upload PHP Shell:**
1. คลิกปุ่ม **Browse**
2. เลือกไฟล์ `.php` ที่เตรียมไว้
3. คลิก **Open** และ **Save**

**2.2 สิ่งที่เกิดขึ้น:**
- หน้าเว็บจะดูเหมือน "แขวน" (Hanging)
- เพราะ Burp กำลัง Intercept Request
- ต้อง Forward Request เอง

**2.3 หา POST Request:**
- Forward Request จนเจอ
- **POST Request ที่มีการอัปโหลดไฟล์**
- จะเห็น PHP Code ในตัว Request

**ขั้นตอนที่ 3: แก้ไข Content-Type**

**ข้อมูลเดิมใน Request:**
```http
...
**📝 หมายเหตุสำคัญ:**
> บาง Payloads มี Comments จาก Author:
> - อธิบายการใช้งาน
> - Kudos และลิงก์ Blog
> - **⚠️ ควรลบออกเพื่อหลบ Detection!**

**ผลลัพธ์:**
- "หลอก" Server ให้คิดว่าเป็นไฟล์ GIF
- **Bypass การตรวจสอบ File Type**
- อัปโหลดไฟล์ `.php` สำเร็จ!

**ขั้นตอนที่ 4: Forward และตรวจสอบ**

1. คลิก **Forward** สองครั้ง
2. ปิด Burp Interceptor
3. กลับไปดูที่ Browser
...
**สิ่งที่เห็น:**
- Vendor Entry ชื่อ **"NetVen"**
- Logo แสดงเป็นรูป **กระดาษขาด**
- หมายความว่า rConfig ไม่รู้จัก File Type
- ใช้รูป Default แทน
- **แต่ไฟล์ .php ถูกอัปโหลดสำเร็จแล้ว!**

## การใช้งาน Web Shell

### เข้าถึง Shell:

**URL ที่ต้อง Navigate:**
```
http://target-ip/images/vendor/connect.php
```

**สิ่งที่เกิดขึ้น:**
1. 🚀 Execute Payload
2. 🖥️ ได้ **Non-interactive Shell Session**
3. 🌐 ทั้งหมดอยู่ใน Browser
4. ⚙️ Execute Commands บน OS ได้

### Interface ของ Web Shell:

**ฟีเจอร์ที่มี:**
- 📁 **Fetch Files** - ดึงไฟล์จากระบบ
- 🌐 **Host/Port Fields** - ตั้งค่าการเชื่อมต่อ
- 📝 **Path Field** - ระบุ Path ของไฟล์
- ⌨️ **Command Execution** - รันคำสั่ง
- 🔐 **Sudo Output** - แสดงคำสั่งที่อนุญาต



## ข้อควรพิจารณาเมื่อใช้ Web Shells

### ปัญหาที่อาจพบ:

**1. การลบไฟล์อัตโนมัติ:**
- ⏰ Web Applications อาจลบไฟล์หลังช่วงเวลาหนึ่ง
- ต้องรีบทำงานให้เสร็จ
- หรือสร้าง Persistence

**2. Limited Interactivity:**
- 🚫 จำกัดการโต้ตอบกับ OS
- ยาก Navigation File System
- ยาก Download/Upload Files
- **Command Chaining อาจไม่ทำงาน:**
  ```bash
  # อาจไม่ได้ผล
  whoami && hostname
  ```
...
**3. ความช้าในการทำงาน:**
- 🐌 ช้ากว่า Interactive Shell
- Enumeration ใช้เวลานาน
- ต้องรอ Response ทุกครั้ง

**4. ความไม่เสถียร:**
- ⚠️ Non-interactive Web Shell ไม่เสถียร
- Connection อาจหลุดง่าย
- ต้องเข้าใหม่บ่อย

**5. หลักฐานที่เหลือทิ้งไว้:**
- 🕵️ **โอกาสถูกตรวจพบสูง**
- เหลือหลักฐานการโจมตีบน Server
- ต้องระมัดระวังและลบร่องรอย

## Best Practices และ Operational Security

### การทำงานอย่างมืออาชีพ:

**1. Upgrade Shell:**
```bash
# แนะนำ: Upgrade เป็น Reverse Shell
# หลังได้ Web Shell แล้ว
# จากนั้นลบ Payload ทันที
```

**2. Stealth Operations:**

**กรณี Evasive Assessment (Black Box):**
- 🤐 พยายามไม่ถูกตรวจจับ
- ปกปิดร่องรอย (Cover Tracks)
- เลียนแบบ Malicious Attacker
- ช่วยทดสอบความสามารถของ Client ในการตรวจจับ
...
- 📄 ชื่อ Payloads และไฟล์ที่ใช้
- 🔐 **SHA1sum หรือ MD5 Hash** ของไฟล์
- 📍 ตำแหน่งที่อัปโหลด
- 📊 รวมใน Reports เป็นหลักฐาน
